...
function Context(source::ValidSources;
    ...
    groupmark::Union{UInt8, Char, Nothing}=nothing,
...
@refargs function Context(source::ValidSources,
    ...
    groupmark::Union{UInt8, Char, Nothing},
    ...
    if groupmark isa Char && !isascii(groupmark)
        throw(ArgumentError("`groupmark` ($groupmark) must be an ASCII character"))
    end
    if !isnothing(groupmark) && groupmark == delim
        throw(ArgumentError("`groupmark` and `delim` must differ"))
    end
