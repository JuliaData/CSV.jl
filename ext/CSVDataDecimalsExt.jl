...
# A requested `Decimal{P, S}` must hold the field's value exactly. The field is
# read once as a `DecimalValue` with the storage type of the target, which keeps
# the digits and the scale the field spelled. The value then moves to the
# target scale: fewer places scale up with an overflow check, more places divide
# and must leave no remainder. A field with more digits than the storage type
# holds, or with an exponent outside the `DecimalValue` range, takes the wide
# path: the spelling decides exactness before the target type parses the field.
