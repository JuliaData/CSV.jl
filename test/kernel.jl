...
@testset "structural: vector masks and prefix XOR" begin
    ...
    if Sys.ARCH === :aarch64
        ...
        @test K.HAS_PMULL[] == (Sys.isapple() || C.test_cpu_feature(C.JL_AArch64_aes))
    end
...
@testset "typed: user-provided types" begin
    ...
    @test_throws CSV.ParseError K.parse("a\n1\nxyz\n"; types=Dict(:a => Int64), on_error=:error)
    ...
    @test err isa CSV.ParseError && occursin("invalid_quoted_field at data row 1", sprint(showerror, err))
    @test_throws ArgumentError K.parse("a\n1\n"; maxproblems=-1)
