...
## Follow-up (2026-08-19) — actions from the review comments on this document

Twelve comment threads; each resolved below with the commit that carries it.
Full battery after all of this: kernel + values + api + write + scan + legacy
green; Tables `jq/scan` pushed to PR #380.

* **#25 / Tables PR** — pushed: PR #380 head is now `ee9df1e` (Scan + `Tables.scan`, no apply/finish).
* **#8 pooling → API layer, kernel pooling deleted** (`aefda15`): −418 lines from core.jl (PoolCtx/PoolSegment/InlineTable/ViewKey, the pooled parse loop, the hash-sampled pre-skip, the pooled merge, all argument threading). `_poolcolumn` interns a finished CompactString column in parallel row ranges (allocation-free hashing) and merges level lists in range order — first-occurrence ids exactly as before. **21.4 ms with `pool=(0.2,500)` on the 39 MiB file vs 22.4 ms kernel-integrated** (pool=false 12.4). One war story on the way: a closure that assigned `levels` in the merge scope raced across tasks — task bodies are named functions, and the boxed-capture audit test now covers CSVApi/KernelWrite too.
* **#11 phase-structured `_decompose` + beat Base on long mantissas** (`bc164b3`): sign → integer run → point → fraction run → SWAR >19-digit tail → exponent, 8 digits per word, span-bounded loads. Field-exact vs the old byte loop (kept as an in-test oracle) over 450k adversarial spans. **40-digit 104 → 70 ns (Base 71, fast_float 75); 100-digit 136 → 38 (fast_float 51!); 400-digit 768 → 381 (Base 589); integer-like 15 → 10.** Every shape now beats Base by 1.5–4×; 9 of 16 at parity-or-better vs fast_float, the rest 1.2–1.9× (Eisel–Lemire micro-tuning, not structure). Table: `floatsweep-2026-08-19.txt`.
* **#17 Dates tokens `I p e E`** (`982459b`): 12-hour hour, case-insensitive AM/PM with `adjusthour` and Dates' 1..12 rule, day names validated-and-ignored. 160k format round trips vs Dates in both cases + adversarial spellings, zero mismatches.
* **#24 bare quotes in skipped rows** (`1f7e335`): prefix rows (before a numbered header, or before `skipto` with no header) are physical lines — quotes inert, never indexed, never sniffed; rows between a header and `skipto` stay quote-aware. The audit's three repros parse.
* **#24 the four cheap wins** (`8c260bb`): `http(s)://` sources via Downloads; a space-aligned file elects `(' ', ignorerepeated=true)` (scored last, only when it cannot change a file that detected before); `select`/`drop` names match raw *or* normalized; a malformed-quote cell keeps its raw bytes *and* reports. Also cleaned the uncommitted Project.toml diff: bench deps live in the test env, InlineStrings is a weakdep again, the package gains only Downloads.
* **#26 writer perf pass + matrix** (`8f47af8`, `bench/writebench.jl`, `writebench-2026-08-19.txt`): specialized `string(x)`-exact float writer (25 → 23 ns; the layer wasn't the bulk — Ryu digit emission is), pairwise int digits (16 → 12 ns), presized in-memory sinks, sampled block sizing. 8T: **3–56× faster than 0.10 on every shape/size; vs polars 1.1–3.1× faster on mixed/strings/datetime at 200k rows, 0.4–0.8× on Float64-heavy shapes and at 1M rows.** Remaining levers, measured: float digit emission (Ryu ~21 ns vs ~10 in polars — a Dragonbox/Schubfach port), first-touch page faults on render buffers; the sink copy is at memcpy speed.
* **#22 CompactStrings.jl, Arrow-adapted** — decided and scaffolded at `~/.julia/dev/CompactStrings` (local git, tests green): payload = Arrow `Utf8View` **exactly** (Int32 length; inline bytes for ≤ 12; else 4-byte prefix + Int32 `buffer_index` + Int32 `offset`), nullness in a separate validity vector, data buffers ≤ 2 GiB, Base-only, String parity for `==`/`cmp`/`hash`/`String`, Dict interop. **CSV adjusts** (follow-up, ~a day): the input buffer becomes data buffer 0 split into ≤ 2 GiB windows (buffer index = window), the escaped-`extra` buffer becomes buffer 1+, the `-1` length sentinel gives way to the validity vector (the kernel already tracks presence bytes for typed columns), and `src/compactstring.jl` becomes `using CompactStrings`. Creating the org repo is yours: `gh repo create JuliaStrings/CompactStrings.jl --public --source ~/.julia/dev/CompactStrings --push`.
* **#24 StreamedSource plan** — `docs/review/streamed-source-plan.md`: ChunkedBase's coordinator (serial IO + tail-shift + `chunkplan` per buffer, row-aligned by construction, exact quote state, `nbuffers × bufsize` memory) feeding the unchanged kernel inner loop; how Rows/Chunks/File-over-IO/gzip use it, the Chunks schema-stability protocol, threading/ordering/backpressure, sizes, and a build order.
* **#17 plan notes** — recorded: no `PosLen` in 3.0 (WeakRefStrings is effectively dead); no `supportedtype`/`typeparser` seam — FixedPointDecimals and ChunkedCSV pin to 2 (you'll work FPD out with its maintainer directly).
* **#25 fused per-chunk filter** — dropped by your call (the two-phase path's mask cost is 0.6 ms on 1M rows; not worth the driver complexity).
